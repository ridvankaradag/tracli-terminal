...
exports.report = () => {
  ...
  database.data.projects.map(project => {
    ...
    project.tasks.map(task => {
      table.push([task.id, task.name, new Date(task.created_at).toDateString(), millisecondToString(calculateTotalTime(task))])

      total_time += calculateTotalTime(task);
    })

    table.push(["","","",millisecondToString(total_time)])
...
createTimeEntry = (args, type) => {
  ...
  if(entries.length === 0 && type !== "start") {
    console.log("You can't pause or finish task before start")
    return false;
  }
  ...
  if(entries.length > 0 && entries[entries.length - 1].type !== "start" && type === "pause") {
    console.log("No active task for pausing")
    return false;
  }
...
statusPattern = (task) => {
  ...
  let pattern = `\nTask: ${task.id} | ${task.name}\nLast activity: ${last_activity.type} on ${new Date(last_activity.created_at).toString()}`

  if(last_activity.type === "start") {
    let timer = new Date(Date.now() - last_activity.created_at)
    pattern += `\n${timer.getUTCHours()}:${timer.getMinutes()}:${timer.getSeconds()}\n`
  }
  return pattern;
}
